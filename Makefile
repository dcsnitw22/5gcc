...
oamctl: ## Build oamctl
	@echo "# installing oamctl ${VERSION}"
...
#       cd cmd/oamctl && go build -ldflags "${LDFLAGS}" -tags="${GO_BUILD_TAGS}" ${GO_BUILD_ARGS}
...
pdusmsp-dev-image: ## Build developer image
	...
	IMAGE_TAG=$(PDUSMSP_DEV_IMAGE_TAG) \
                PDUSMSP_IMG=$(PDUSMSP_IMG) PDUSMSP_VERSION=$(PDUSMSP_VERSION)  \
                BUILD_DATE=$(BUILD_DATE) \
                DOCKERFILE_DIR=$(PDUSMSP_DEV_DOCKERFILE_DIR) \
          ./build.sh
