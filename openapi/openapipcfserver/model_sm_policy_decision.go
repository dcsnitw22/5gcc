...
func (s SmPolicyDecision) MarshalBinary() ([]byte, error) {
	return json.Marshal(s)
}

// AssertSmPolicyDecisionRequired checks if the required fields are not zero-ed
func AssertSmPolicyDecisionRequired(obj SmPolicyDecision) error {
	if err := AssertChargingInformationRequired(obj.ChargingInfo); err != nil {
		return err
	}
	if obj.PolicyCtrlReqTriggers != nil {
		for _, el := range *obj.PolicyCtrlReqTriggers {
			if err := AssertPolicyControlRequestTriggerRequired(el); err != nil {
				return err
			}
		}
	}
	for _, el := range obj.LastReqRuleData {
		if err := AssertRequestedRuleDataRequired(el); err != nil {
			return err
		}
	}
	if err := AssertRequestedUsageDataRequired(obj.LastReqUsageData); err != nil {
		return err
	}
...
// AssertSmPolicyDecisionConstraints checks if the values respects the defined constraints
func AssertSmPolicyDecisionConstraints(obj SmPolicyDecision) error {
	return nil
}
