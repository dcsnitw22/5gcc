...
func (a *SessionManagementSubscriptionDataAPIService) QuerySmDataExecute(r ApiQuerySmDataRequest) (*SmSubsData, *http.Response, error) {
	fmt.Println("requst coming from udm server to udr client")
	var (
	...
	fmt.Println("Yes api of server is called")
	...
	var tmp ExtendedSmSubsData
	err = a.client.decode(&tmp, localVarBody, localVarHTTPResponse.Header.Get("Content-Type"))
	if err != nil {
		newErr := &GenericOpenAPIError{
			body:  localVarBody,
			error: err.Error(),
