...
// AssertAmInfluDataRequired checks if the required fields are not zero-ed
func AssertAmInfluDataRequired(obj AmInfluData) error {
	for _, el := range obj.DnnSnssaiInfos {
		if err := AssertDnnSnssaiInformationRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.EvSubs {
		if err := AssertAmInfluEventRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.CovReq {
		if err := AssertServiceAreaCoverageInfoRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertAmInfluDataConstraints checks if the values respects the defined constraints
func AssertAmInfluDataConstraints(obj AmInfluData) error {
	return nil
}
