...
// AssertTrafficDescriptorComponentsRequired checks if the required fields are not zero-ed
func AssertTrafficDescriptorComponentsRequired(obj TrafficDescriptorComponents) error {
	for _, el := range obj.EthFlowDescs {
		if err := AssertEthFlowDescriptionRequired(el); err != nil {
			return err
		}
	}
	for _, el := range obj.ConnCaps {
		if err := AssertConnectionCapabilitiesRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertTrafficDescriptorComponentsConstraints checks if the values respects the defined constraints
func AssertTrafficDescriptorComponentsConstraints(obj TrafficDescriptorComponents) error {
	return nil
}
