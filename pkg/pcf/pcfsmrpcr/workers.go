...
type SessionMessageInfo struct {
	...
	PcfsmrpcrMsg   api.SessionMessage // Message
}

type PcfsmrpcrpSessionWorkers struct {
...
func NewSessionWorkers(function SessionFunction, cache cache.WorkCache, backoffInterval time.Duration) SessionWorkers {
	return &PcfsmrpcrpSessionWorkers{
...
func (p *PcfsmrpcrpSessionWorkers) HandleSessionMessages(msgInfo *SessionMessageInfo) {
...
func (p *PcfsmrpcrpSessionWorkers) sessionWorkerHandler(sessionMessages <-chan SessionMessageInfo) {
...
func (p *PcfsmrpcrpSessionWorkers) RemoveSessionWorkers(deletedSessions map[SessionId]struct{}) error {
	p.sessionLock.Lock()
	defer p.sessionLock.Unlock()
