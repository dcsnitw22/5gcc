...
func NewPdusmsp(cfg *config.PdusmspConfig, time time.Time) (PdusmspBootstrap, bool) {
	...
	// Intialize tracer
	// pdusmsp.tracerManager = &tracing.TracerManager{}
	// Intialize grpc
...
func (p *Pdusmsp) Run(configChannel <-chan config.PdusmspConfig) {
	...
	// start the tracer manager
	// go p.tracerManager.Start()
	...
	for {
		...
		case pdusmsMsg := <-sessionChannel:
			...
			case sm.NSMF_CREATE_SM_CONTEXT_REQUEST:
				...
				// ctx, createSpan := tracing.Tracer.Start(ctx, "CreateSMContextSetup")
				...
				// Ensure the request is not nil before accessing its context
				if pdusmsMsg.Request == nil {
