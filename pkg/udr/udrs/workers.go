...
type SessionMessageInfo struct {
	...
	UdrsMsg        api.SessionMessage // Message
}

type UdrspSessionWorkers struct {
...
func (p *UdrspSessionWorkers) HandleSessionMessages(msgInfo *SessionMessageInfo) {
...
func (p *UdrspSessionWorkers) sessionWorkerHandler(sessionMessages <-chan SessionMessageInfo) {
...
func (p *UdrspSessionWorkers) RemoveSessionWorkers(deletedSessions map[SessionId]struct{}) error {
	p.sessionLock.Lock()
	defer p.sessionLock.Unlock()
